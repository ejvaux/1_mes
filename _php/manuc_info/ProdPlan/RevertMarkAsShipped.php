...
else
{
    ...
    $lotnumber = $_POST['lotnumber'];
        ...
        if($drno!="UNASSIGNED DR")
        {
            ...
            (SELECT packing_number FROM mis_dr_assigned WHERE item_code = '$itemcode' AND dr_number = '$drno')";
            $result = $conn->query($sql);
        ...
        {
            $sql = "UPDATE mis_product SET SHIP_STATUS='APPROVED' WHERE PACKING_NUMBER IN 
            (SELECT packing_number FROM mis_dr_assigned WHERE item_code = '$itemcode' AND group_name = '$grno')";
            $result = $conn->query($sql);
