<?php
  ...
  if($stmt = $conn1->query("SELECT DATE_, SUM(PLAN_QTY), COUNT(DATE_) FROM mis_prod_plan_dl WHERE DATE_ between '$from' and '$to' and JOB_ORDER_NO like'2%' group by DATE_")){
   echo "<table id='table_query' border = '2' ><tr align = 'center'> <th width = '100px'>DATE</th>"; 
...
//$php_data_array[] = $row;
  ...
  $tdef=0;
  if($stmt = $conn2->query("SELECT COUNT(created_at) FROM defect_mats WHERE created_at BETWEEN '$from%' and '$to%' group by DATE(updated_at)")){
