...
<tr align = 'center' ><strong> $Linename (Day)</strong> <th width = '100px' style='position: absolute;
    display: flex;  background: #fff;'>DATE</th><td style='  margin-left: 90px;'></td>"; 
      for ($fromstart; $fromstart <=$toend ; $fromstart++) { 
if($stmt = $conn1->query("SELECT DATE_, SUM(PLAN_QTY) FROM mis_prod_plan_dl WHERE DATE_ >='$start' AND DATE_ADD(DATE_, INTERVAL 1 DAY) <='$end' and JOB_ORDER_NO like'8%' and MACHINE_CODE='$line'  ")){
...
$tplan=0;  
  echo "<tr align = 'center'> <th width = '100px' style='position: absolute;
    ...
    if($stmt = $conn1->query("SELECT COUNT(DATE_), SUM(PLAN_QTY) FROM mis_prod_plan_dl WHERE DATE_ >='$start1' AND DATE_ADD(DATE_, INTERVAL 1 DAY) <='$end1' and JOB_ORDER_NO like'8%' and MACHINE_CODE like '$line' ")){
...
$tgap=0;$i=0; //------------------------------------ 
  ...
  like '8%'   and  shift = '$shift'  and type = '1' and PDLINE_NAME like '$line' ")){
...
$trate=0;    $i=0; //------------------------------------ 
   ...
   if($stmt = $conn2->query("SELECT COUNT(RESULT) FROM pcb WHERE created_at>='$start3' AND DATE_ADD(created_at, INTERVAL 0 DAY) <='$end3' and jo_number like '8%' 
   and  shift = '$shift'  and type = '1' and PDLINE_NAME like '$line' ")){
...
echo "<tr align = 'center'> <th width = '100px' style='position: absolute;
 ...
 for ($fromstart5; $fromstart5 <=$toend5 ; $fromstart5++) { 
  ...
  WHERE created_at>='$start5' AND DATE_ADD(created_at, INTERVAL 0 DAY) <='$end5'
   AND jo_number LIKE '8%' and shift = '$shift' and PDLINE_NAME like '$line' and PROCESS_NAME  like 'DIP.INPUT%' ")){
...
and jo_number like '8%' and PROCESS_NAME  like 'DIP.INPUT%' and PDLINE_NAME like '$line' and shift = '$shift' group by cast(created_at + 0.25 as date)")){
