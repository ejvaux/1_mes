...
if($stmt = $conn2->query("SELECT  count(defect_id), date(created_at) FROM defect_mats WHERE  division_id='18' AND created_at>='$start4' AND DATE_ADD(created_at, INTERVAL 0 DAY) <='$end4'    ")){
...
echo "<tr align = 'center'> <th width = '100px' style='position: absolute;
 ...
 for ($fromstart5; $fromstart5 <=$toend5 ; $fromstart5++) { 
  ...
  WHERE created_at>='$start5' AND DATE_ADD(created_at, INTERVAL 0 DAY) <='$end5'
   AND jo_number LIKE '8%'   and PROCESS_NAME  like 'DIP.INPUT%' ")){
...
and jo_number like '8%' and PROCESS_NAME  like 'DIP.INPUT%'  group by cast(created_at + 0.25 as date)")){
