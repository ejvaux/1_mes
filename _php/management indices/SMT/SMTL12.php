...
<tr>
  ...
  <tr align = 'center'> <th width = '100px' style='position: absolute;
    ...
    if($stmt = $conn1->query("SELECT COUNT(DATE_), SUM(PLAN_QTY) FROM mis_prod_plan_dl WHERE DATE_ >='$start1' AND DATE_ADD(DATE_, INTERVAL 1 DAY) <='$end1' and JOB_ORDER_NO like'2%' AND MACHINE_CODE LIKE 'SMTL12' and SALES_ORDER NOT LIKE 'M%' ")){while ($plan = $stmt->fetch_row()){
    $tplan+=$plan[1];
...
$end2=date('Y-m-d H:i:s',strtotime($_POST['from'].'+1 days'.' 05:59:59' ));
  $tresult=0;
   ...
   if($stmt = $conn2->query("SELECT sum(type) FROM pcb WHERE created_at>='$start2' AND DATE_ADD(created_at, INTERVAL 0 DAY) <='$end2' and jo_number like '2%' 
     and type = '1' AND line_id='17' and `work_order` NOT LIKE 'M%' ")){
