...
<tr>
  ...
  <tr align = 'center'> <th width = '100px' style='position: absolute;
    ...
    if($stmt = $conn1->query("SELECT SUM(PLAN_QTY) FROM mis_prod_plan_dl WHERE DATE_ >='$start1' AND DATE_ADD(DATE_, INTERVAL 1 DAY) <='$end1' and JOB_ORDER_NO like'8%' AND MACHINE_CODE LIKE 'DIPL2' ")){while ($plan = $stmt->fetch_row()){
    $tplan+=$plan[0];
 echo "<td style='font-size:12px; height:20px;'>".number_format(@$plan[0],0,'.',',')."</td>";
    $job_array[]=$plan[0];
//$php_data_array[] = $row;
...
if($stmt = $conn2->query("SELECT SUM(division_id) FROM defect_mats WHERE division_id='18' AND created_at>='$start4' AND DATE_ADD(created_at, INTERVAL 0 DAY) <='$end4'   AND line_id='20' ")){
