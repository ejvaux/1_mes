...
//$php_data_array[] = $row;
  ...
  echo "<td><b>".number_format($tplan,0,'.',',')."<b></td></tr>";}
   ...
   on masterdatabase.mis_prod_plan_dl.JOB_ORDER_NO = 1_smt.pcb.jo_number where masterdatabase.mis_prod_plan_dl.DATE_ BETWEEN '$from' and '$to' and 1_smt.pcb.jo_number like '2%' 
    and 1_smt.pcb.shift = '1' and 1_smt.pcb.type = '1'  group by masterdatabase.mis_prod_plan_dl.DATE_")){
...
$tgap=0; //------------------------------------ 
  ...
  like '2%'  and 1_smt.pcb.shift = '1' group by masterdatabase.mis_prod_plan_dl.DATE_")){
...
$trate=0; //------------------------------------ 
  ...
  while ($rate = $stmt->fetch_row()){
   ...
   // echo $gp[1]."//".$gp[2]."///";
    ...
    $tdef=0;
if($stmt = $conn2->query("SELECT COUNT(1_smt.defect_mats.created_at), 1_smt.defect_mats.updated_at FROM 1_smt.defect_mats WHERE 1_smt.defect_mats.created_at BETWEEN '$from 06%' and '$to 17%' and 1_smt.defect_mats.shift = '1' group by DATE(1_smt.defect_mats.created_at)")){
...
$tinput=0;
  ...
  WHERE masterdatabase.mis_prod_plan_dl.JOB_ORDER_NO = 1_smt.pcb.jo_number AND masterdatabase.mis_prod_plan_dl.DATE_ BETWEEN '$from' AND '$to'
   AND 1_smt.pcb.jo_number LIKE '2%' and 1_smt.pcb.shift = '1'  GROUP BY masterdatabase.mis_prod_plan_dl.DATE_")){
...
WHERE masterdatabase.mis_prod_plan_dl.JOB_ORDER_NO = 1_smt.pcb.jo_number and masterdatabase.mis_prod_plan_dl.DATE_ between '$from' and '$to' 
and 1_smt.pcb.jo_number like '2%' and 1_smt.pcb.PROCESS_NAME  like 'SMT.INPUT%' and PDLINE_NAME like 'SMTL%' and 1_smt.pcb.shift = '1' group by masterdatabase.mis_prod_plan_dl.DATE_")){
