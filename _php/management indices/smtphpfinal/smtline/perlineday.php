...
<tr align = 'center' ><td rowspan='9' width = '100px'><h4 style='margin-top:80%; font-size:auto;'>$line</h4><i>(6ap)</i></td> </tr><tr align = 'center'> <th width = '100px'>DATE</th>"; 
...
//$php_data_array[] = $row;
  ...
  echo "<td><b>".number_format($tplan,0,'.',',')."<b></td></tr>";}
   ...
   and  1_smt.pcb.shift = '$shift'  and 1_smt.pcb.type = '1' and PDLINE_NAME like '$line' group by cast(1_smt.pcb.created_at + 0.25 as date) ")){
   echo "<tr align = 'center'> <th width = '100px'>PROD RESULT</th>";
   $i=0;
  while ($result = $stmt->fetch_row()){
...
$tgap=0; //------------------------------------ 
  ...
  like '2%'   and  1_smt.pcb.shift = '$shift'  and 1_smt.pcb.type = '1' and PDLINE_NAME like '$line' group by cast(1_smt.pcb.created_at + 0.25 as date)")){
...
$trate=0; //------------------------------------ 
  ...
  while ($rate = $stmt->fetch_row()){
   ...
   // echo $gp[1]."//".$gp[2]."///";
    ...
    $tdef=0;
if($stmt = $conn2->query("SELECT COUNT(1_smt.defect_mats.created_at), 1_smt.defect_mats.updated_at FROM 1_smt.defect_mats WHERE cast(1_smt.pcb.created_at + 0.25 as date) BETWEEN '$from' and '$to'  and  1_smt.pcb.shift = '$shift'  and 1_smt.pcb.type = '1'  group by cast(1_smt.pcb.created_at + 0.25 as date)")){
...
$tinput=0;
  ...
  WHERE masterdatabase.mis_prod_plan_dl.JOB_ORDER_NO = 1_smt.pcb.jo_number AND cast(1_smt.pcb.created_at + 0.25 as date) BETWEEN '$from' AND '$to'
   AND 1_smt.pcb.jo_number LIKE '2%' and 1_smt.pcb.shift = '$shift' and PDLINE_NAME like '$line' and 1_smt.pcb.PROCESS_NAME  like 'SMT.INPUT%' GROUP BY cast(1_smt.pcb.created_at + 0.25 as date)")){
...
WHERE masterdatabase.mis_prod_plan_dl.JOB_ORDER_NO = 1_smt.pcb.jo_number and cast(1_smt.pcb.created_at + 0.25 as date) between '$from' and '$to' 
and 1_smt.pcb.jo_number like '2%' and 1_smt.pcb.PROCESS_NAME  like 'SMT.INPUT%' and PDLINE_NAME like '$line' and 1_smt.pcb.shift = '$shift' group by cast(1_smt.pcb.created_at + 0.25 as date)")){
