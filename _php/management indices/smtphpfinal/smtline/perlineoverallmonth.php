...
$tplan=0;
    if($stmt = $conn1->query("SELECT MONTH(DATE_), SUM(PLAN_QTY) FROM mis_prod_plan_dl WHERE DATE_ between '$from' and '$to' and JOB_ORDER_NO like'2%' and MACHINE_CODE LIKE '$line' group by MONTH(DATE_)")){
...
//$php_data_array[] = $row;
  ...
  echo "<td><b>".number_format($tplan,0,'.',',')."<b></td></tr>";}
   ...
   and PROCESS_NAME like 'SMT.INPUT%' and 1_smt.pcb.PDLINE_NAME='$line'  group by MONTH(masterdatabase.mis_prod_plan_dl.DATE_)")){
...
$tgap=0; //------------------------------------ 
  if($stmt = $conn1->query("SELECT MONTH(masterdatabase.mis_prod_plan_dl.DATE_), masterdatabase.mis_prod_plan_dl.PLAN_QTY, COUNT(1_smt.pcb.RESULT) FROM masterdatabase.mis_prod_plan_dl, 1_smt.pcb 
  ...
  like '2%' and 1_smt.pcb.PDLINE_NAME='$line' group by MONTH(masterdatabase.mis_prod_plan_dl.DATE_)")){
...
$trate=0; //------------------------------------ 
  ...
  while ($rate = $stmt->fetch_row()){
   ...
   // echo $gp[1]."//".$gp[2]."///";
    ...
    $tdef=0;
if($stmt = $conn2->query("SELECT COUNT(1_smt.defect_mats.created_at), 1_smt.defect_mats.updated_at FROM 1_smt.defect_mats WHERE 1_smt.defect_mats.created_at BETWEEN '$from 00%' and '$to 23%' group by MONTH(1_smt.defect_mats.created_at)")){
...
$tinput=0;
  ...
  WHERE masterdatabase.mis_prod_plan_dl.JOB_ORDER_NO = 1_smt.pcb.jo_number AND masterdatabase.mis_prod_plan_dl.DATE_ BETWEEN '$from' AND '$to'
   AND 1_smt.pcb.jo_number LIKE '2%' and 1_smt.pcb.PDLINE_NAME='$line'  GROUP BY MONTH(masterdatabase.mis_prod_plan_dl.DATE_)")){
...
$yield;
$tyield=0;
if($stmt = $conn1->query("SELECT MONTH(masterdatabase.mis_prod_plan_dl.DATE_), COUNT(1_smt.pcb.RESULT), COUNT(1_smt.pcb.PROCESS_NAME) FROM masterdatabase.mis_prod_plan_dl, 1_smt.pcb
WHERE masterdatabase.mis_prod_plan_dl.JOB_ORDER_NO = 1_smt.pcb.jo_number and masterdatabase.mis_prod_plan_dl.DATE_ between '$from' and '$to' 
and 1_smt.pcb.jo_number like '2%' and 1_smt.pcb.PROCESS_NAME  like 'SMT.INPUT%' and 1_smt.pcb.PDLINE_NAME='$line' group by MONTH(masterdatabase.mis_prod_plan_dl.DATE_)")){
