...
if($stmt = $conn1->query("SELECT DATE_, SUM(PLAN_QTY) FROM mis_prod_plan_dl WHERE DATE_ >='$start' AND DATE_ADD(DATE_, INTERVAL 1 DAY) <='$end' and JOB_ORDER_NO like'2%' and MACHINE_CODE='$line2'  ")){
    
  
  while ($date = $stmt->fetch_row()) {
       echo "<tr align = 'center' ><td>".$line2."</td>";
...
$end2=date('Y-m-d H:i:s',strtotime($_POST['from'].'+1 days'.' 05:59:59' ));
  $tresult=0;
   ...
   if($stmt = $conn2->query("SELECT count(id), Month(created_at),day(created_at),time(created_at), created_at FROM pcb WHERE created_at>='$start2' AND DATE_ADD(created_at, INTERVAL 0 DAY) <='$end2' and jo_number like '2%' 
     and type = '1' and PDLINE_NAME like '$line2' ")){
   $i=0;
  while ($result = $stmt->fetch_row()){
 echo "<td>".number_format($result[0],0,'.',',') ."</td>";
 $tresult+=$result[0];
      $result_arrayyy[] = $result[0];
$php_data_arrayyy[] =$result[0];   $i++;
...
$tgap=0;$i=0; //------------------------------------ 
  ...
  like '2%'     and type = '1' and PDLINE_NAME like '$line2' ")){
  while ($gp = $stmt->fetch_row()){
    $job_arrayyy[$i];
    $gp[0]=$result_arrayyy[$i];
    //echo $gp[1]."//".$gp[2]."///";
    $gap = $job_arrayyy[$i] - $gp[0];
...
if($stmt = $conn2->query("SELECT COUNT(created_at), updated_at FROM defect_mats WHERE division_id='2' AND created_at>='$start4' AND DATE_ADD(created_at, INTERVAL 0 DAY) <='$end4' and line_id='$line_id2'  ")){

while ($def = $stmt->fetch_row()){
    $defect_arrayyy[]=$def[0];
