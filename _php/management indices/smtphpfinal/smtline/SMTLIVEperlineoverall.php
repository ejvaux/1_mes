...
<tr align = 'center' ><th>LINE NO.</th> <th width = '100px' >PLAN</th><th width = '100px'>TARGET</th><th width = '100px' >RESULT</th><th width = '100px'>GAP</th><th width = '100px'>DEFECT</th></tr><table  class='table table-bordered table-sm' >"; 
...
if($stmt = $conn1->query("SELECT DATE_, SUM(PLAN_QTY) FROM mis_prod_plan_dl WHERE DATE_ >='$start' AND DATE_ADD(DATE_, INTERVAL 1 DAY) <='$end' and JOB_ORDER_NO like'2%' and MACHINE_CODE='$line'  ")){
  ...
  while ($date = $stmt->fetch_row()) {
       echo "<tr align = 'center' ><td>".$line."</td>";
