...
//$php_data_array[] = $row;
  ...
  echo "<td><b>".number_format($tplan,0,'.',',')."<b></td></tr>";}
   ...
   where cast(created_at + 0.25 as date) BETWEEN  '$from' and '$to' and jo_number like '2%' 
     and type = '1' and PDLINE_NAME like '$line' group by cast(created_at + 0.25 as date) ")){
...
$tgap=0; //------------------------------------ 
  ...
  like '2%'     and type = '1' and PDLINE_NAME like '$line' group by cast(created_at + 0.25 as date)")){
...
$tdef=0;
if($stmt = $conn2->query("SELECT COUNT(created_at), updated_at FROM defect_mats WHERE cast(created_at + 0.25 as date) BETWEEN '$from' and '$to'     group by cast(created_at + 0.25 as date)")){
...
$tinput=0;
  ...
  WHERE    cast(created_at + 0.25 as date) BETWEEN '$from' AND '$to'
   AND jo_number LIKE '2%' and PDLINE_NAME like '$line' and PROCESS_NAME  like 'SMT.INPUT%' GROUP BY cast(created_at + 0.25 as date)")){
...
WHERE  cast(created_at + 0.25 as date) between '$from' and '$to' 
and jo_number like '2%' and PROCESS_NAME  like 'SMT.INPUT%' and PDLINE_NAME like '$line' group by cast(created_at + 0.25 as date)")){
