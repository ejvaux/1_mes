...
<div align = "center">
<label><b>PRODUCTION SUMMARY OF <i> DIP </i></b></label></div>
...
if($stmt = $conn->query("SELECT DATE_, SUM(PLAN_QTY), SUM(PROD_RESULT) FROM mis_prod_plan_dl Where DATE_ between '$from' and '$to' and JOB_ORDER_NO like '7%' group by DATE_ ")){
...
// over all total of date range,, CASE STATEMENT
if($stmt = $conn->query("SELECT MONTH(DATE_), SUM(PLAN_QTY), SUM(PROD_RESULT) FROM mis_prod_plan_dl Where DATE_ between '$mfrom' and '$mto' and JOB_ORDER_NO like '7%' group by MONTH(DATE_)")){
