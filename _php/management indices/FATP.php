...
<div align = "center">
<label><b>PRODUCTION SUMMARY OF <i>FATP </i></b></label>
...
if($stmt = $conn1->query("SELECT DATE_, SUM(PLAN_QTY) FROM mis_prod_plan_dl WHERE DATE_ between '$from' and '$to' and JOB_ORDER_NO like'3%' or '6%' group by DATE_")){
...
and mis_prod_plan_dl.DATE_ between '$from' and '$to' and mis_prod_plan_dl.JOB_ORDER_NO like'3%' group by mis_prod_plan_dl.DATE_")){
...
<?php
    ...
    if($stmt = $conn1->query("SELECT MONTH(DATE_), SUM(PLAN_QTY) FROM mis_prod_plan_dl WHERE DATE_ between '$mfrom' and '$mto' and JOB_ORDER_NO like'3%' group by MONTH(DATE_))")){
     echo "<table border = '2' ><tr align = 'center'> <th width = '100px'>DATE</th>"; 
    while ($row = $stmt->fetch_row()) {
      echo "<td>$row[0]</td>";
     //$php_data_array[] = $row;
    ...
    and mis_prod_plan_dl.DATE_ between '$mfrom' and '$mto' and mis_prod_plan_dl.JOB_ORDER_NO like'3%' group by MONTH(mis_prod_plan_dl.DATE_)")){
     echo "<tr align = 'center'> <th width = '100px'>PROD RESULT</th>";
    while ($row = $stmt->fetch_row()){
       echo "<td>$row[2]</td>";
       $php_data_array[] = $row;}
