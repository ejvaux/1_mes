...
and mis_prod_plan_dl.DATE_ between '$from' and '$to' AND MACHINE_CODE LIKE '".$MG."%' and mis_prod_plan_dl.JOB_ORDER_NO LIKE '1%'   group by mis_prod_plan_dl.DATE_")){
...
$tgap=0;
if($stmt = $conn1->query("SELECT mis_prod_plan_dl.DATE_, SUM(mis_prod_plan_dl.PLAN_QTY), SUM(mis_summarize_results.PROD_RESULT) FROM mis_prod_plan_dl, mis_summarize_results WHERE mis_prod_plan_dl.JOB_ORDER_NO = mis_summarize_results.JOB_ORDER_NO 
and mis_prod_plan_dl.DATE_ between '$from' and '$to' AND mis_prod_plan_dl.MACHINE_CODE LIKE '".$MG."%' and mis_prod_plan_dl.JOB_ORDER_NO LIKE '1%'  group by mis_prod_plan_dl.DATE_")){
echo "<tr align = 'center' > <th width = '100px' style='position: absolute;
...
$tinput=0;
if($stmt = $conn1->query("SELECT SUM(mis_product.ACTUAL_QTY) FROM mis_product, mis_prod_plan_dl WHERE mis_product.JO_NUM = mis_prod_plan_dl.JOB_ORDER_NO AND mis_prod_plan_dl.DATE_ between '$from' and '$to' AND mis_prod_plan_dl.MACHINE_CODE LIKE '".$MG."%' and mis_prod_plan_dl.JOB_ORDER_NO LIKE '1%'   group by mis_prod_plan_dl.DATE_")){
